            ...
            <div>
              <span
                className={cn(
                 ...
                 isTransparent ? "text-gradient" : "text-purple-500",
                )}
              >
                DreamCarta
              </span>
              <div
                className={cn(
                  "text-[10px] -mt-1 font-body tracking-wider transition-all duration-300",
                  isTransparent ? "text-purple-500/60" : "text-muted-foreground",
                )}
              >
                LIFE BLUEPRINT
              </div>
          ...
          <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <button
                  ...
                  className={cn(
                    ...
                    isTransparent ? "hover:bg-primary/10" : "hover:bg-white/10",
                  )}
                >
                  <img
                    src={
                      user.avatar ||
                      `https://ui-avatars.com/api/?name=${user.name}&background=6133B4&color=fff`
                    }
                    alt={user.name}
                  ...
                  <span
                    className={cn(
                      ...
                      isTransparent ? "text-gray-800 dark:text-white" : "text-white",
                    )}
                  >
                    {user.name.split(" ")[0]}
                  </span>
                  <ChevronDown
                    className={cn(
                      ...
                      isTransparent ? "text-muted-foreground" : "text-white/70",
                    )}
                  />
                </button>
                <AnimatePresence>
                    ...
                    >
                      ...
                      <div className="p-2">
                        ...
                        <button
                          ...
                          className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors"
                        >
                          Logout
                        </button>
                      </div>
            ...
            <button
              ...
              className={cn(
                ...
                scrolled
                  ? "text-gray-800 dark:text-white hover:bg-muted"
              ...
              {!user ? (
                <div className="flex flex-col gap-2 pt-2">
                  <Link
                    to="/login"
                    onClick={() => setMobileOpen(false)}
                    className="block px-4 py-3 text-center text-primary border border-primary rounded-lg text-sm font-medium"
                  >
                    Login
                  </Link>
                  <Link
                ...
                <div className="pt-2 border-t border-border">
                  {user.role === "admin" ? (
                    <>
                      <Link
                        to="/admin"
                        onClick={() => setMobileOpen(false)}
                        className="block px-4 py-3 text-sm hover:bg-muted rounded-lg"
                      >
                        Admin Panel
                      </Link>
                      <Link
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-3 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg"
                  >
                    Logout
                  </button>
                </div>
